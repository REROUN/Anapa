  ...
  override fun draw(canvas: Canvas) { // Type 변수 받아오기
    ...
    if (selectedModel == "앞으로 들기") {
      ...
      if (rightShoulder != null && rightHip != null) {
        modifiedRightHipX = rightShoulder.getPosition().x
        rightHipY = rightHip.getPosition().y
      }
...
//            ) / 180 * 100
            val rightHipAngle = getAngle3D(
              ...
              rightShoulder!!.getPosition3D(),
              rightWrist!!.getPosition3D()
            )
...
//      drawLine(canvas, rightKnee, rightAnkle, rightPaint)
      ...
      canvas.drawLine(translateX(rightShoulder!!.position3D.x), translateY(rightShoulder!!.position3D.y), translateX(300.0f),
        translateY(rightShoulder!!.position3D.y), rightPaint)
