...
class PoseGraphicLeft internal constructor(
    ...
    override fun draw(canvas: Canvas) { // Type 변수 받아오기
        ...
        if (selectedModel == "앞으로 들기") {
            for (landmark in landmarks) { // Draw all the points
                if (landmark.landmarkType == PoseLandmark.LEFT_WRIST || landmark.landmarkType == PoseLandmark.LEFT_ELBOW
                ...
                for (landmark in landmarks) {
                    if (landmark.landmarkType == PoseLandmark.LEFT_SHOULDER) {
                        val rightHipAnglerightHipAngle = getAngle(
                            extractLandmarkFromType(pose, PoseLandmark.LEFT_HIP),
                            extractLandmarkFromType(pose, PoseLandmark.LEFT_SHOULDER),
                            extractLandmarkFromType(pose, PoseLandmark.LEFT_WRIST)
                        )
...
//            val rightHip = pose.getPoseLandmark(PoseLandmark.RIGHT_HIP)
            ...
            if (leftShoulder != null && leftHip != null) {
                modifiedRightHipX = leftShoulder.getPosition().x
                rightHipY = leftHip.getPosition().y
            }
            ...
            if (showInFrameLikelihood) {
                for (landmark in landmarks) {
                    if (landmark.landmarkType == PoseLandmark.RIGHT_SHOULDER) {
                        val modifiedRightHipPosition = PointF3D.from(modifiedRightHipX, leftHip!!.getPosition().y, leftHip.getPosition3D().z)
...
//            ) / 180 * 100
            ...
            for (landmark in landmarks) { // Draw all the points
                if (landmark.landmarkType == PoseLandmark.LEFT_WRIST || landmark.landmarkType == PoseLandmark.LEFT_SHOULDER
                    || landmark.landmarkType == PoseLandmark.LEFT_HIP) {
                    drawPoint(canvas, landmark, whitePaint)
                }
                if (visualizeZ && rescaleZForVisualiz) {
...
//      drawLine(canvas, rightKnee, rightAnkle, rightPaint)
            ...
            canvas.drawLine(translateX(leftShoulder!!.position3D.x), translateY(leftShoulder!!.position3D.y), translateX(300.0f),
                translateY(leftShoulder!!.position3D.y), rightPaint)
...
//            val rightWrist = pose.getPoseLandmark(PoseLandmark.RIGHT_WRIST)
            val leftHip = pose.getPoseLandmark(PoseLandmark.LEFT_HIP)
//            val rightHip = pose.getPoseLandmark(PoseLandmark.RIGHT_HIP)
//      val leftR = pose.getPoseLandmark(PoseLandmark.LEFT_KNEE)
//      val rightKnee = pose.getPoseLandmark(PoseLandmark.RIGHT_KNEE)
//      val leftAnkle = pose.getPoseLandmark(PoseLandmark.LEFT_ANKLE)
//      val rightAnkle = pose.getPoseLandmark(PoseLandmark.RIGHT_ANKLE)
...
//            )
                        ...
                        val rightElbowrightShoulderleftShoulderAngle = getAngle(
                            extractLandmarkFromType(pose, PoseLandmark.LEFT_ELBOW),
                            extractLandmarkFromType(pose, PoseLandmark.LEFT_SHOULDER),
                            extractLandmarkFromType(pose, PoseLandmark.RIGHT_SHOULDER)
                        )
...
//    drawLine(canvas, leftHip, leftKnee, leftPaint)
//    drawLine(canvas, leftKnee, leftAnkle, leftPaint)
//    drawLine(canvas, leftWrist, leftPulgar, leftPaint)
//    drawLine(canvas, leftWrist, leftMeñique, leftPaint)
//    drawLine(canvas, leftWrist, leftIndice, leftPaint)
//    drawLine(canvas, leftIndice, leftMeñique, leftPaint)
//    drawLine(canvas, leftAnkle, leftTacon, leftPaint)
...
//    drawLine(canvas, rightHip, rightKnee, rightPaint)
//    drawLine(canvas, rightKnee, rightAnkle, rightPaint)
//    drawLine(canvas, rightWrist, rightPulgar, rightPaint)
//    drawLine(canvas, rightWrist, rightMeñique, rightPaint)
//    drawLine(canvas, rightWrist, rightIndice, rightPaint)
//    drawLine(canvas, rightIndice, rightMeñique, rightPaint)
//    drawLine(canvas, rightAnkle, rightTacon, rightPaint)
